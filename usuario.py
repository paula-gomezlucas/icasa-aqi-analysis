...
class usuario:
    ...
    def acceder(self):
        ...
        if datos:
            ...
            if hashlib.md5(self.passw.encode()).hexdigest() == acceso[0][0]:
                ...
                lista = conbbdd.queryBBDD(mydb, comprobarautorizacion)
    ...
    def registroBBDD(self):
        ...
        if datos:
            ...
            conbbdd.queryBBDD(mydb, registroUsuario)

    def accesoBBDD(self, fila):
        query = "SELECT EMAIL, NOMBRE FROM USUARIO WHERE EMAIL = '{fila}';".format(fila=fila)
        mydb = conbbdd.conectar('195.235.211.197', 'psi_grupo2', 'psi_grupo2', '1234')
        respuesta = conbbdd.queryBBDD(mydb, query)
        conbbdd.desconectar(mydb)
        return respuesta
